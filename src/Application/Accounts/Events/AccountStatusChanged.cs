        ...
        public class EventHandler(ILogger<EventHandler> logger) : INotificationHandler<Notification>
        {
            public Task Handle(Notification notification, CancellationToken cancellationToken)
            {
                logger.LogInformation(
                    "Account {AccountId} status changed {Previous} -> {New} by {ActorId} (correlation {CorrelationId}).",
                    notification.AccountId, notification.PreviousStatus, notification.NewStatus,
                    notification.ActorId, notification.CorrelationId);
