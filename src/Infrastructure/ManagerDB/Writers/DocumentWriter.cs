...
public sealed class DocumentWriter(IApplicationDbContext context, ICurrentPrincipal principal) : AccountScopedDataAccess(context, principal), IDocumentWriter
{
    ...
    public async Task MarkDocumentUploadedAsync(Guid documentId, string status, CancellationToken cancellationToken) => await UpdateDocumentAsync(documentId, x => x.Status = status, cancellationToken);
    public async Task MarkDocumentScanResultAsync(Guid documentId, string scanStatus, CancellationToken cancellationToken) => await UpdateDocumentAsync(documentId, x => x.ScanStatus = scanStatus, cancellationToken);
    public async Task ExpireDocumentAsync(Guid documentId, DateTimeOffset expiresAt, CancellationToken cancellationToken) => await UpdateDocumentAsync(documentId, x => { x.ExpiresAt = expiresAt; x.Status = "Expired"; }, cancellationToken);
    public async Task DeleteDocumentReferenceAsync(Guid documentId, CancellationToken cancellationToken) => await UpdateDocumentAsync(documentId, x => x.Status = "Deleted", cancellationToken);
